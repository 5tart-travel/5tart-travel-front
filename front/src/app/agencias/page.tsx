...
import ModalFormularioTravel from "@/components/Agencias/AñadirTravel/ModalPostTravel";
...
const Agencia = () => {
  ...
  const handleFilter = (ubicacion: string) => {
  ...
  const filtrarAgencias = () => {
    return agencias.filter(agencia => {
      return filtroUbicacion ? agencia.location!.toLowerCase().includes(filtroUbicacion.toLowerCase()) : true;
    });
  };
    ...
    <main className='bg-gray-50 flex flex-col items-center'>
      ...
      <Suspense fallback={<div>Cargando...</div>}>
        {filteredAgencias.length > 0 ? (
          <ListaAgencias agencias={filteredAgencias} />
        ) : (
...
//   useEffect(() => {
//     fetch('/api/agencias') // Actualiza la URL a tu API local
//       .then(response => response.json())
//       .then((data: IAgencias[]) => {
//         setAgencias(data);
...
//         zonas={[]} // Si no estás usando zonas, puedes dejarlo vacío
//       />
