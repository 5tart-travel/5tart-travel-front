...
const CardAgencia: React.FC<{ agencia: IAgencias }> = ({ agencia }) => {
  const truncatedDescription = truncateDescription(agencia.description!, 55);
  ...
  return (
    <div className="bg-transparent border-t-4 border-lime500 rounded-xl shadow-xl p-4 m-2 md:m-4 max-w-xs mx-auto transform transition-transform duration-200 hover:scale-105">
      <div className="h-64 relative overflow-hidden rounded-t-lg">
        <Image src={agencia.image!} alt={agencia.agency!} layout="fill" objectFit="cover" />
      </div>
      <div className="mt-4">
        <p className="mt-2 text-gray-700 dark:text-gray-400">{agencia.location}</p>
        <p className="mt-2 text-gray-700 dark:text-gray-400">{truncatedDescription}</p>
        <a href={`/agencias/${agencia.id}`} className="border border-lime500 inline-block mt-4 px-4 py-2 text-sm font-semibold text-gray-800 rounded-xl hover:bg-lime500 focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2">
          Leer más
        </a>
      </div>
    </div>
