  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/tours/mailOfertas`,
        {
          method: 'POST',
          headers: {
      ...
      setMessage(
        'Hubo un error al intentar registrarse. Por favor, intentá de nuevo más tarde.',
      );
    }
  };
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center py-8 px-4 ${
        tema ? 'bg-grey' : 'bg-gray-100'
      } `}
    >
      <p
        className={`mb-4 text-lg font-semibold ${
          tema ? 'text-white' : 'text-gray-800'
        }  text-center`}
      >
      ...
      >
        ...
        <button
          ...
          } rounded-b-3xl sm:rounded-br-3xl sm:rounded-b-none hover:bg-blue-900 focus:outline-none`}
        >
          Registrate
        </button>
      </form>
